...
public final class RingGroups {
  ...
  public static boolean isUpToDate(RingGroup ringGroup, Collection<DomainAndVersion> domainAndVersions) throws IOException {
    for (Ring ring : ringGroup.getRings()) {
      if (!Rings.isUpToDate(ring, domainAndVersions)) {
        return false;
      }
    }
    return true;
